...
Select<T_Consumer, T_Predicate, T_Relation>::Select(
	...
	: _nextOp(aConsOp), _pred(aPredicate), _vectorizedSize(1), _indexNo1(), _indexNo2()
{}
...
Select<T_Consumer, T_Predicate, T_Relation>::Select(
	...
	: _nextOp(aConsOp), _pred(aPredicate), _vectorizedSize(aVectorizedSize), _indexNo1(), _indexNo2()
{}
...
Select<T_Consumer, T_Predicate, T_Relation>::step(unval_vt& aTupel, T_Relation& aRelation, const size_t aSize, const bool aNoMoreData) 
{
	...
	if(_pred(aTupel[_indexNo1]._unval_pt, aTupel[_indexNo2]._unval_pt, lTupleSize, _vectorizedSize, aRelation))
	{
	...
	{
		aTupel[_indexNo2]._unval_pt[_vectorizedSize]._size = _pred.getOutputIndex();
		_nextOp->step(aTupel, aRelation, _vectorizedSize, aNoMoreData);
	}
}
